...
void processCommand(const char* command) {
  ...
  // 라즈베리파이 클라이언트 호환 - "c" 또는 "command" 필드 지원
  ...
  if (doc.containsKey("c")) {
    ...
    sendError("No command field found");
    return;
  }
...
void executeCommand(const char* cmdType) {
  // 라즈베리파이 클라이언트 호환 - 짧은 명령어 지원
  if (strcmp_P(cmdType, PSTR("sensor")) == 0 || strcmp_P(cmdType, PSTR("get_sensor_data")) == 0) {
    handleSensorDataRequest();
  } else if (strcmp_P(cmdType, PSTR("led")) == 0 || strcmp_P(cmdType, PSTR("set_led")) == 0) {
    handleLedControl();
  } else if (strcmp_P(cmdType, PSTR("servo")) == 0 || strcmp_P(cmdType, PSTR("control_servo")) == 0) {
    handleServoControl();
  } else if (strcmp_P(cmdType, PSTR("stop")) == 0 || strcmp_P(cmdType, PSTR("stop_servo")) == 0) {
    handleServoStop();
  } else if (strcmp_P(cmdType, PSTR("clean")) == 0 || strcmp_P(cmdType, PSTR("cage_cleaning")) == 0) {
    handleCageCleaning();
  } else if (strcmp_P(cmdType, PSTR("reset")) == 0 || strcmp_P(cmdType, PSTR("reset_emergency_stop")) == 0) {
    handleEmergencyReset();
  } else if (strcmp_P(cmdType, PSTR("reset_cycles")) == 0 || strcmp_P(cmdType, PSTR("reset_cleaning_cycles")) == 0) {
    handleCleaningCyclesReset();
  } else if (strcmp_P(cmdType, PSTR("status")) == 0 || strcmp_P(cmdType, PSTR("get_status")) == 0) {
    sendStatus();
  } else if (strcmp_P(cmdType, PSTR("test")) == 0 || strcmp_P(cmdType, PSTR("system_test")) == 0) {
    handleSystemTest();
  } else if (strcmp_P(cmdType, PSTR("trash")) == 0 || strcmp_P(cmdType, PSTR("trash_empty_button")) == 0) {
    handleTrashEmptyButton();
  } else if (strcmp_P(cmdType, PSTR("ping")) == 0) {
    // 연결 테스트용 ping 명령 추가
    sendResponse("pong");
  } else {
...
void handleServoControl() {
  // 라즈베리파이 클라이언트 호환 - "d" 또는 "direction" 지원
  int direction = 0;
  ...
  controlServo(direction); // 0: 정지, 1: 앞으로, -1: 뒤로
...
// ========== 쓰레기통 비우기 버튼 처리 (전류 감지) ==========
void handleTrashEmptyButton() {
  // 현재 전류 상태와 자동 초기화 상태 확인
  bool currentPowerState = digitalRead(TRASH_EMPTY_BUTTON_PIN);
  
  doc.clear();
  doc["response"] = "Trash system status";
  doc["power_connected"] = currentPowerState;
  doc["auto_reset_triggered"] = systemStatus.trash_empty_button_pressed;
...
// ========== 쓰레기통 비우기 버튼 읽기 (전류 감지 방식) ==========
void readTrashEmptyButton() {
  static bool lastPowerState = LOW;  // 이전 전류 상태
  static bool powerWasOff = false;   // 전류가 끊어진 적이 있는지
  static unsigned long powerOffTime = 0;  // 전류가 끊어진 시간
  static unsigned long lastStateChange = 0;  // 마지막 상태 변경 시간
  const unsigned long debounceDelay = 100;   // 디바운싱 시간
  const unsigned long resetDelay = 10000;    // 10초 대기 시간
  
  // 현재 전류 상태 읽기 (HIGH = 전류 흐름, LOW = 전류 끊김)
  bool currentPowerState = digitalRead(TRASH_EMPTY_BUTTON_PIN);
  ...
  if (currentPowerState != lastPowerState) {
    if (millis() - lastStateChange > debounceDelay) {
      ...
      if (currentPowerState == LOW && lastPowerState == HIGH) {
        ...
        powerWasOff = true;
        powerOffTime = millis();
        Serial.println(F("{\"info\":\"POWER_DISCONNECTED_TRASH_SYSTEM\"}"));
        
      } else if (currentPowerState == HIGH && lastPowerState == LOW && powerWasOff) {
        // 전류가 다시 연결되었을 때
        unsigned long offDuration = millis() - powerOffTime;
        
        if (offDuration >= resetDelay) {
          // 10초 이상 끊어진 후 다시 연결되면 청소 횟수 초기화
          systemStatus.cleaning_cycles = 0;
          systemStatus.trash_full = false;
          systemStatus.trash_empty_button_pressed = true;
          Serial.println(F("{\"alert\":\"CLEANING_CYCLES_AUTO_RESET\",\"off_duration\":"));
          Serial.print(offDuration);
          Serial.println(F("}"));
        } else {
          Serial.println(F("{\"info\":\"POWER_RECONNECTED_TOO_EARLY\",\"off_duration\":"));
          Serial.print(offDuration);
          Serial.println(F(",\"required\":10000}"));
        }
